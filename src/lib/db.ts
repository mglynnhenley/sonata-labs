...
const g = globalThis as unknown as { __slackSandboxWorking?: Db };
...
function openWorking(): Db {
  ...
  // ATTACH audit.db so mutation + audit row commit atomically in one
...
/** The working workspace connection, with `audit` ATTACHed. */
export function getDb(): Db {
  if (!g.__slackSandboxWorking) g.__slackSandboxWorking = openWorking();
  return g.__slackSandboxWorking;
}

/** Close the working connection (used by reset before swapping files). */
export function closeWorkingDb(): void {
  if (g.__slackSandboxWorking) {
    g.__slackSandboxWorking.close();
    g.__slackSandboxWorking = undefined;
  }
}

/** Apply the full workspace schema to an already-open handle (snapshot/working). */
