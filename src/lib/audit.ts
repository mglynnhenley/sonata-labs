...
// the mutation + history bump — and still survives a reset (separate file).

const g = globalThis as unknown as { __gmailSandboxSession?: string };

/** Ensure a session exists and return its id (creates one lazily). */
export function getCurrentSessionId(db: Database): string {
  if (g.__gmailSandboxSession) {
    const exists = db
      .prepare("SELECT 1 FROM audit.sessions WHERE id = ?")
      .get(g.__gmailSandboxSession);
    if (exists) return g.__gmailSandboxSession;
  }
 ...
 * atomically with the mailbox change. `at` lets tests pin the timestamp.
 */
...
export function listActions(
  ...
  return db
    .prepare(
      `SELECT * FROM audit.action_log ${where} ORDER BY id DESC LIMIT ?`,
    )
