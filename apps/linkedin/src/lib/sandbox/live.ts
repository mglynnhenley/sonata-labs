...
// goes on serving the old, now-nameless inode. That is how a company page with
// six seeded posts answers the control plane with empty arrays: the twin is not
// wrong about its own state, it is looking at a file nobody else can see any
// more.
//
...
const g = globalThis as unknown as { __linkedinSandboxWorkingInode?: number };
...
export function liveDb(): Database {
  ...
  if (g.__linkedinSandboxWorkingInode !== inode) closeWorkingDb();
  const db = getDb();
  // Read back after opening: a missing file has just been created by getDb().
  g.__linkedinSandboxWorkingInode = inodeOf(WORKING_PATH);
  return db;
}
