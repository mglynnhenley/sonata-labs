...
// audit.db DDL, addressable via the ATTACH alias `audit`. This is a hand-mirror
// of the audit section of db/schema.sql, because those tables have to be created
// against the alias on a brand-new file. Change one and you must change the
// other — nothing catches the drift, and the symptom is a mutation that commits
// while its audit row silently does not.
...
// The key is clone-specific on purpose: two clones sharing one key inside a
// single process would hand each other the wrong database.
const g = globalThis as unknown as { __linkedinSandboxWorking?: Db };
...
/** The working LinkedIn connection, with `audit` ATTACHed. */
export function getDb(): Db {
  if (!g.__linkedinSandboxWorking) g.__linkedinSandboxWorking = openWorking();
  return g.__linkedinSandboxWorking;
}

/** Close the working connection (used by reset before swapping files). */
export function closeWorkingDb(): void {
  if (g.__linkedinSandboxWorking) {
    g.__linkedinSandboxWorking.close();
    g.__linkedinSandboxWorking = undefined;
  }
}

/** Apply the full schema to an already-open handle (snapshot/working). */
