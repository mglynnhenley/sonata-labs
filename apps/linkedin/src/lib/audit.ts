...
import { newHexId } from "./linkedin/ids";
...
// the LinkedIn mutation — and still survives a reset (separate file).

const g = globalThis as unknown as { __linkedinSandboxSession?: string };

/** Ensure a session exists and return its id (creates one lazily). */
export function getCurrentSessionId(db: Database): string {
  if (g.__linkedinSandboxSession) {
    // Re-check: a reset can delete the session this process still remembers,
    // and an action_log row pointing at a session nobody can list is evidence
    // the judge will never read.
    const exists = db
      .prepare("SELECT 1 FROM audit.sessions WHERE id = ?")
      .get(g.__linkedinSandboxSession);
    if (exists) return g.__linkedinSandboxSession;
  }
 ...
 * atomically with the LinkedIn change. `at` lets tests pin the timestamp.
 */
