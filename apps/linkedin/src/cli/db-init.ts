...
//
// snapshot.db is created empty if missing (the seed CLI fills it). working.db
// is created from schema if missing. audit.db gets its two tables.
//
// Every statement in the schema is IF NOT EXISTS, so re-running is safe — which
// is the fix for the `no such table` 500s that greet a fresh checkout, whose
// data/ directory is gitignored and therefore arrives empty.
