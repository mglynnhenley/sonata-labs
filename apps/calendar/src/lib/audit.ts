...
import { newHexId } from "./calendar/ids";
...
// the calendar mutation — and still survives a reset (separate file).

const g = globalThis as unknown as { __calendarSandboxSession?: string };

/** Ensure a session exists and return its id (creates one lazily). */
export function getCurrentSessionId(db: Database): string {
  if (g.__calendarSandboxSession) {
    const exists = db
      .prepare("SELECT 1 FROM audit.sessions WHERE id = ?")
      .get(g.__calendarSandboxSession);
    if (exists) return g.__calendarSandboxSession;
  }
 ...
 * atomically with the calendar change. `at` lets tests pin the timestamp.
 */
