...
// goes on serving the old, now-nameless inode. That is how a workspace with five
// seeded documents answers `/api/sandbox/snapshot` with an empty array: the twin
...
const g = globalThis as unknown as { __googleDocsSandboxWorkingInode?: number };
...
export function liveDb(): Database {
  ...
  if (g.__googleDocsSandboxWorkingInode !== inode) closeWorkingDb();
  const db = getDb();
  // Read back after opening: a missing file has just been created by getDb().
  g.__googleDocsSandboxWorkingInode = inodeOf(WORKING_PATH);
  return db;
}
