...
// the document mutation — and still survives a reset (separate file).

const g = globalThis as unknown as { __googleDocsSandboxSession?: string };

/** Ensure a session exists and return its id (creates one lazily). */
export function getCurrentSessionId(db: Database): string {
  if (g.__googleDocsSandboxSession) {
    // Re-checked rather than trusted: a reset swaps the database out from under
    // this process, and a cached id that no longer exists would write action
    // rows the activity endpoint can never group.
    const exists = db
      .prepare("SELECT 1 FROM audit.sessions WHERE id = ?")
      .get(g.__googleDocsSandboxSession);
    if (exists) return g.__googleDocsSandboxSession;
  }
...
export function startNewSession(db: Database, note: string, at?: number): string {
  ...
  g.__googleDocsSandboxSession = id;
  return id;
}
 ...
 * atomically with the document change. `at` lets tests pin the timestamp.
 */
