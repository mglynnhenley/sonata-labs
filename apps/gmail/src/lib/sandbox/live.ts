...
// a mailbox with 45 seeded messages answers `/api/sandbox/snapshot` with
...
const g = globalThis as unknown as { __gmailSandboxWorkingInode?: number };
...
export function liveDb(): Database {
  ...
  if (g.__gmailSandboxWorkingInode !== inode) closeWorkingDb();
  const db = getDb();
  // Read back after opening: a missing file has just been created by getDb().
  g.__gmailSandboxWorkingInode = inodeOf(WORKING_PATH);
  return db;
}
