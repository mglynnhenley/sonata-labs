...
const g = globalThis as unknown as { __gmailSandboxWorking?: Db };
...
function openWorking(): Db {
  ...
  // ATTACH audit.db so mutation + history bump + audit row commit atomically in
  // one transaction, while audit.db remains a separate file that survives resets.
...
/** The working mailbox connection, with `audit` ATTACHed. */
export function getDb(): Db {
  if (!g.__gmailSandboxWorking) g.__gmailSandboxWorking = openWorking();
  return g.__gmailSandboxWorking;
}

/** Close the working connection (used by reset before swapping files). */
export function closeWorkingDb(): void {
  if (g.__gmailSandboxWorking) {
    g.__gmailSandboxWorking.close();
    g.__gmailSandboxWorking = undefined;
  }
}

/** Apply the full mailbox schema to an already-open handle (snapshot/working). */
