...
// replaces working.db from OUTSIDE this process — `npm run seed`, `npm run
// sync`, a restore by hand — unlinks that file and writes a new one, and the
// open handle happily goes on serving the old, now-nameless inode. That is how
// a workspace with 45 seeded messages answers `/api/sandbox/snapshot` with
// empty arrays: the twin is not wrong about its own state, it is looking at a
// file nobody else can see any more.
//
...
const g = globalThis as unknown as { __slackSandboxWorkingInode?: number };
...
export function liveDb(): Database {
  ...
  if (g.__slackSandboxWorkingInode !== inode) closeWorkingDb();
  const db = getDb();
  // Read back after opening: a missing file has just been created by getDb().
  g.__slackSandboxWorkingInode = inodeOf(WORKING_PATH);
  return db;
}
