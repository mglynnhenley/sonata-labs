...
import { newHexId } from "./attio/ids";
...
// the CRM mutation — and still survives a reset (separate file).

const g = globalThis as unknown as { __attioSandboxSession?: string };

/** Ensure a session exists and return its id (creates one lazily). */
export function getCurrentSessionId(db: Database): string {
  if (g.__attioSandboxSession) {
    // Re-check: a reset swaps the file underneath us and can take the cached
    // session with it, leaving every later action row orphaned.
    const exists = db
      .prepare("SELECT 1 FROM audit.sessions WHERE id = ?")
      .get(g.__attioSandboxSession);
    if (exists) return g.__attioSandboxSession;
  }
 ...
 * atomically with the CRM change. `at` lets tests pin the timestamp.
 */
