...
// POST /api/sandbox/seed — load a cloned advertiser account:
// { twin: "google-ads", seed }. The contract is written out in full at the top
// of packages/world/src/inject.ts.
//
...
export async function POST(req: Request) {
  ...
  try {
    const result = seedFromWire(parseSeedRequest(await req.json().catch(() => null)));
    // A promote closed and reopened the handle; recording it stops liveDb()
    // reopening a second time on the next control-plane read.
    markWorkingSwapped();
