...
// ATTACH so the two tables always exist even on a brand-new file — and yes, the
// two copies drift silently, because nothing checks them against each other.
...
// The key is unique per clone: two twins sharing one Node process would
// otherwise hand each other the wrong database.
const g = globalThis as unknown as { __googleAdsSandboxWorking?: Db };
...
/** The working account connection, with `audit` ATTACHed. */
export function getDb(): Db {
  if (!g.__googleAdsSandboxWorking) g.__googleAdsSandboxWorking = openWorking();
  return g.__googleAdsSandboxWorking;
}

/** Close the working connection (used by reset before swapping files). */
export function closeWorkingDb(): void {
  if (g.__googleAdsSandboxWorking) {
    g.__googleAdsSandboxWorking.close();
    g.__googleAdsSandboxWorking = undefined;
  }
}
